...
internal class IssueTrackerTransformerTest {
    @Test
    fun `date should be parsed correctly`() {
        val dateString = "2020-04-07T08:30:48.589+0300"
        val date = IssueTrackerTransformer.dateFormatter.parse(dateString)
        println(DateTimeFormatter.ISO_OFFSET_DATE_TIME.format(date))
        println(IssueTrackerTransformer.dateFormatter.format(date))
        println(IssueTrackerTransformer.dateFormatter.format(ZonedDateTime.now()))
    }
