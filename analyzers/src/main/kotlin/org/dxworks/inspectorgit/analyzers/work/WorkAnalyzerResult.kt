...
class WorkAnalyzerResultDTO {
    ...
    companion object {
        fun get(result: WorkAnalyzerResult): WorkAnalyzerResultDTO {
            ...
            dto.commitId = result.commit.id
            dto.authorId = result.commit.author.id
            dto.newWork = result.newWork.map { AnnotatedLineDTO(it.commit.id, it.number, it.content) }
            dto.churn = result.churn.map { AnnotatedLineDTO(it.commit.id, it.number, it.content) }
...
class CodeChangeDTO {
    ...
    companion object {
        fun get(codeChange: CodeChange): CodeChangeDTO {
            ...
            dto.addedLine = AnnotatedLineDTO(addedLine.commit.id, addedLine.number, addedLine.content)
            val removedLine = codeChange.removedLine
            dto.removedLine = AnnotatedLineDTO(removedLine.commit.id, removedLine.number, removedLine.content)
            return dto
        }
    }
}
