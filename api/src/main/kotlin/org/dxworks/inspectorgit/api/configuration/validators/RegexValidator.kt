...
import org.dxworks.inspectorgit.api.configuration.exceptions.ValidationFailedException
import java.util.*
        ...
        fields.forEach {
            try {
                if (!properties.getProperty(it).matches(regex))
                    throw ValidationFailedException("In ${this.javaClass.simpleName}: $it did not match ${regex.pattern}")
            } catch (e: IllegalStateException) {
                throw throw ValidationFailedException("Missing field $it")
            }
        }
    }
}